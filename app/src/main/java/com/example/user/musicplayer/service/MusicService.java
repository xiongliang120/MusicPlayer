package com.example.user.musicplayer.service;
...
public class MusicService extends Service {
    ...
    private List<MediaItem> mp3Infos;   //存放Mp3Info对象的集合
    ...
    public static final String MUSIC_PREPARE = "com.wwj.action.MUSIC_PREPARE";//新音乐准备好了
...
//        mp3Infos = MediaUtil.getMp3Infos(PlayerService.this);
    ...
    public class MyBinder extends Binder {
        public void setData(List<MediaItem> data) {
            MusicService.this.mp3Infos = data;
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (msg == MusicConstant.PLAY_MSG) {    //直接播放音乐
            Intent sendIntent = new Intent(UPDATE_ACTION);
            sendIntent.putExtra("current", current);
            // 发送广播，将被Activity组件中的BroadcastReceiver接收到
            sendBroadcast(sendIntent);
            play(0);
        } else if (msg == MusicConstant.PAUSE_MSG) {    //暂停
            pause();
        } else if (msg == MusicConstant.STOP_MSG) {     //停止
            stop();
        } else if (msg == MusicConstant.CONTINUE_MSG) { //继续播放
            resume();
        } else if (msg == MusicConstant.PRIVIOUS_MSG) { //上一首
            previous();
        } else if (msg == MusicConstant.NEXT_MSG) {     //下一首
            next();
        } else if (msg == MusicConstant.PROGRESS_CHANGE) {  //进度更新
            currentTime = intent.getIntExtra("progress", -1);
            play(currentTime);
        } else if (msg == MusicConstant.PLAYING_MSG) {
    ...
    private final class PreparedListener implements MediaPlayer.OnPreparedListener {
        ...
        public void onPrepared(MediaPlayer mp) {
            Intent prepareIntent = new Intent(MUSIC_PREPARE);
